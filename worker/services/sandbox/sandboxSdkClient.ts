...
export class SandboxSdkClient extends BaseSandboxService {
    ...
    private async startDevServer(instanceId: string, _initCommand: string, port: number): Promise<string> {
        try {
            ...
            try {
                const isReady = await this.waitForServerReady(instanceId, process.id, 60000);
                if (isReady) {
        ...
        try {
            ...
            if (installResult.exitCode === 0) {
                ...
                try {
                    ...
                    if (isDev(env) && !env.USE_TUNNEL_FOR_PREVIEW) {
                        previewURL = `http://localhost:${allocatedPort}`;
                        this.logger.info('Forcing localhost preview URL', { instanceId, previewURL });
                    } else if (env.USE_TUNNEL_FOR_PREVIEW) {
